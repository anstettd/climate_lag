...
n_MATA_sla_lag2 <- lmer(SLA ~  Drought*MATA_lag2 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=north)
...
lrtest(n_CMDA_sla_lag2,n_CMDA_sla_lag2a) #no evidence of difference, select simpler model
...
lrtest(n_CMDA_sla_no_lag,n_CMDA_sla_no_drought) #very strong evidenc for only drought 
...
n_CMDA_fl_lag1_no_lag <- lmer(Experiment_Date ~  Drought + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=y9)
n_CMDA_fl_lag1_no_drought <- lmer(Experiment_Date ~  CMDA_lag1 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=y9)
...
lrtest(c_CMDA_sla_lag0,c_CMDA_sla_lag0a) # 2-way moderate evidence 

#SLA lag1
c_CMDA_sla_lag1a <- lmer(SLA ~  Drought+CMDA_lag1 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=centre)
lrtest(c_CMDA_sla_lag1,c_CMDA_sla_lag1a) #2-way moderate evidence
...
lrtest(c_CMDA_sla_no_lag,c_CMDA_sla_no_drought) # no lag
...
lrtest(c_CMDA_fl_nolag,c_CMDA_fl_lag0a_nodrought) #no lag
...
s_sla_lag01 <- lmer(SLA ~  Drought*lag01 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=south)
...
s_fl_lag2 <- lmer(Experiment_Date ~  Drought*lag2 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=south)
...
s_fl_lag1_nodrought <- lmer(Experiment_Date ~  lag1 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=south)
lrtest(s_fl_lag1_nolag,s_fl_lag1_nodrought) # strong evidence for no lag
...
lrtest(s_MATA_fl_lag2,s_MATA_fl_lag2a) #2-way evidence
...
lrtest(s_MAPA_fl_lag2,s_MAPA_fl_lag2a) #2-way interaction significant evidence

#fl lag0,1,2
s_MAPA_fl_lag012a <- lmer(Experiment_Date ~  Drought+MAPA_lag012 + (1|Family) + (1|Block) + (1|Year) + (1|Site_Lat), data=south)
lrtest(s_MAPA_fl_lag012,s_MAPA_fl_lag012a) #2-way interaction significant evidence
